...
type() : 소괄호 내에 있는 데이터(argument)가 어떤 자료형인지 알려주는 명령어
    - JS에서는 typeof가 함수가 아니라 연산자이다.
'''
print('python 수업을 시작했습니다. 파이팅')
print('''
이렇게 다중으로 작성하고 싶을 때는 \'\'\'\'\'\'으로 작성하는 방법도 있다. 
Java에서는 줄 넘어갈 때 마다 + 로 연결해줘야 하는데 그런 점은 편하다.
''')
...
'''
python은 Java나 JS와 다르게 맨 처음이 str이면 뒤의 int/float을 알아서 바꿔주는것을 하지 않는다
그대 사용하는 형변환 함수(conversion)이 있는데
str() : 다른 데이터를 문자열 자료형으로 바꿔주는 함수
int(): 문자열 / 실수 자료형을 정수형으로 바꿔주는 함수 / Jva : (int)"1.3;
float() : 실수 자료형으로 바꿔주는 함수
'''
print(f'안녕하세요 제 이름은 {name}이고, 나이는 {age}살 입니다.')
f'''
전에 JS에서 비슷한 개념이 있었다는 것을 알 수 있다.
console.log(`안녕하세요. 제 이름은 ${name}이고. 나이는 ${age}살입니다.`);의 python판 이라고 생각하면 된다.)
